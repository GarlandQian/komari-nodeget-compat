...
- `extension-traffic` 配额模式在周期基线初始化后能精确提供当前周期总用量；基线尚未初始化时回落到 NodeGet 的真实累计流量，不再显示错误的 `0B`。上下行方向只能按原始累计流量比例拆分。
- 主题没有有效的 `homepagePingBindings` 分配时，适配层会按统一规则把每个节点自动绑定到一个最近有真实样本的 Ping/TCPing 任务；任务发现兼容全局和按节点授权的只读 Token，已有的非空显式绑定始终优先，不生成模拟延迟。
